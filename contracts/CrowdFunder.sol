...
contract CrowdFunder {
   ...
   function createCampaigns(address _owner, string memory _title, string memory _description, uint256 _targetAmount) public returns (uint256) {
   ...
   function donateToCampaigns(uint256 _campaignId) public payable {
       ...
       uint256 donatableAmount = campaign.targetAmount - campaign.amountDonated;

       require(amount != 0, "Your donation should be greater than 0");
       require(amount <= donatableAmount, "Your donation exceeds the requested donation target");
   ...
   function deleteCampaigns(uint256 _campaignId) public {
        require(campaigns[_campaignId].owner == msg.sender, "Only the campaign owner can delete a campaign");
